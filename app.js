...
(function () {
  ...
  var timeTicksEl = document.getElementById("time-ticks");
  ...
  var LOOP_START_KEY = "sunny-loop-start"; /* opt49: 0–1 fraction of timeline */
  var _sunnySavedLoopStartFrac = null;
  ...
  try {
    var savedLs = localStorage.getItem(LOOP_START_KEY);
    if (savedLs != null && savedLs !== "") {
      var lsN = Number(savedLs);
      if (isFinite(lsN) && lsN >= 0 && lsN <= 1) _sunnySavedLoopStartFrac = lsN;
    }
  } catch (eLs) {}
  ...
  var map = new maplibregl.Map({
    ...
    try {
      ...
      var src = "";
      try {
        if (e && e.error && e.error.url) src = String(e.error.url);
        else if (e && e.sourceId) src = String(e.sourceId);
      } catch (eSrc) {}
      var blob = msg + " " + src;
      if (msg.indexOf("Could not load image") !== -1) return;
      if (msg.indexOf("Failed to fetch") !== -1) return;
      if (msg.indexOf("Not Found") !== -1) return;
      if (/\b404\b/.test(blob)) return;
      if (/gibs\.earthdata\.nasa\.gov/i.test(blob)) return;
      if (/tilecache\.rainviewer\.com/i.test(blob)) return;
      if (/AjaxError|ImageError|source.*error/i.test(blob) && /\.png|wmts|tile/i.test(blob)) return;
    } catch (eIgn) {}
  });
  ...
  var GOES_AVAIL_LAG_MS = 55 * 60 * 1000; /* opt40: wall−55m — 40m still 404ed unpublished slots */
  ...
  function goesLatestIsoSync() {
    ...
    /* opt40: once probed, NEVER tip newer than last successful slot */
    if (goesLastGoodIso) {
      var gMs = goesIsoMs(goesLastGoodIso);
      if (isFinite(gMs)) {
        var tipMs = Math.min(gMs, wallMs);
        return toGoesIso(tipMs);
      }
    }
  ...
  function buildCloudTimes() {
    ...
    /* opt40: never include times newer than last successful probe */
    if (goesLastGoodIso) {
      var gMs = snapGoesMs(goesIsoMs(goesLastGoodIso));
      if (isFinite(gMs) && latest > gMs) latest = gMs;
    }
  ...
  function applyGoesLatestToTimeline(iso) {
    ...
    if (atEnd) cloudIndex = sliderMax();
      ...
      else cloudIndex = nearestCloudIndex(keepIso);
    } else {
      cloudIndex = clampIndex(cloudIndex);
    }
    playheadFrac = cloudIndex;
    if (loopStartInited) {
      loopStartFrac = clampLoopStart(loopStartFrac);
  ...
  function nearestCloudIndex(iso) {
    var want = goesIsoMs(iso);
  ...
  function formatTickClock(isoOrDate) {
    var d = (isoOrDate instanceof Date) ? isoOrDate : new Date(isoOrDate);
    if (!isFinite(d.getTime())) return "";
    return d.toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" });
  }

  function pickTickIndices(max) {
    var n, i, idx, out, seen;
    if (!isFinite(max) || max <= 0) return [0];
    n = 5;
    try {
      if (typeof window !== "undefined" && window.innerWidth < 700) n = 4;
    } catch (eN) {}
    if (max + 1 < n) n = max + 1;
    if (n > 6) n = 6;
    if (n < 2) return [0, max];
  ...
  function rebuildTimeTicks() {
    if (!timeTicksEl) return;
    var max, i, ix, frac, lab, edge, html, sig, idxs, fr;
    if (typeof radarOn === "function" && radarOn() && radarPast && radarPast.length) {
      max = radarSliderMax();
      sig = "r|" + max + "|" + (radarPast[0] && radarPast[0].time) + "|" + (radarPast[max] && radarPast[max].time);
    } else if (cloudTimes && cloudTimes.length) {
      max = sliderMax();
      sig = "c|" + max + "|" + cloudTimes[0] + "|" + cloudTimes[max];
    } else {
      if (timeTicksSig === "empty") return;
      timeTicksSig = "empty";
      timeTicksEl.innerHTML = "";
      return;
    }
    if (sig === timeTicksSig) return;
    timeTicksSig = sig;
    idxs = pickTickIndices(max);
    html = "";
    for (i = 0; i < idxs.length; i++) {
      ix = idxs[i];
      frac = max <= 0 ? 0 : (100 * ix / max);
      edge = (i === 0) ? " is-start" : (i === idxs.length - 1 ? " is-end" : "");
      if (typeof radarOn === "function" && radarOn() && radarPast && radarPast[ix]) {
        fr = radarPast[ix];
        lab = formatTickClock(new Date(Number(fr.time) * 1000));
      } else {
        lab = formatTickClock(cloudTimes[ix]);
      }
      html += '<span class="time-tick' + edge + '" style="left:' + frac.toFixed(2) + '%">';
      html += '<span class="time-tick-mark"></span><span class="time-tick-lab">' + lab + "</span></span>";
    }
    timeTicksEl.innerHTML = html;
  }
  ...
  function defaultCloudLoopStartIndex() {
  ...
  function defaultRadarLoopStartIndex() {
  ...
  /* compat aliases used by older call sites — return user loopStartFrac */
  function playLoopStartIndex() {
    ensureLoopStartInited();
    return clampLoopStart(loopStartFrac);
  }
  ...
  function loopStartMs() {
    ensureLoopStartInited();
    var f = clampLoopStart(loopStartFrac);
    if (typeof radarOn === "function" && radarOn() && radarPast && radarPast.length) {
      var i = clampRadarIndex(Math.round(f));
      var fr = radarPast[i];
      return fr && fr.time != null ? Number(fr.time) * 1000 : NaN;
    }
    if (cloudTimes.length) {
      var iso = cloudTimes[clampIndex(Math.round(f))];
      return goesIsoMs(iso);
    }
    return NaN;
  }

  function persistLoopStart() {
    var max = activeSliderMax();
    var frac01 = max <= 0 ? 0 : (clampLoopStart(loopStartFrac) / max);
    if (frac01 < 0) frac01 = 0;
    if (frac01 > 1) frac01 = 1;
    try { localStorage.setItem(LOOP_START_KEY, String(frac01)); } catch (eP) {}
    savedLoopStartFrac = frac01;
  }

  function ensureLoopStartInited() {
    if (loopStartInited) {
      loopStartFrac = clampLoopStart(loopStartFrac);
      return;
    }
    var max = activeSliderMax();
    if (max <= 0 && !(cloudTimes && cloudTimes.length) && !(radarPast && radarPast.length)) {
      loopStartFrac = 0;
      return;
    }
    if (savedLoopStartFrac != null && isFinite(savedLoopStartFrac)) {
      loopStartFrac = clampLoopStart(savedLoopStartFrac * max);
    } else {
      var def = (typeof radarOn === "function" && radarOn() && radarPast && radarPast.length)
        ? defaultRadarLoopStartIndex()
        : defaultCloudLoopStartIndex();
      loopStartFrac = clampLoopStart(def);
    }
    loopStartInited = true;
  }

  function setLoopStartFrac(f, opts) {
    opts = opts || {};
    ensureLoopStartInited();
    loopStartFrac = clampLoopStart(f);
    if (playheadFrac < loopStartFrac) {
      playheadFrac = loopStartFrac;
      if (typeof radarOn === "function" && radarOn()) {
        radarIndex = clampRadarIndex(Math.round(playheadFrac));
      } else {
        cloudIndex = clampIndex(Math.round(playheadFrac));
      }
    }
    if (!opts.skipPersist) persistLoopStart();
    syncLoopSplitter();
    syncSliderUi();
  }
  ...
  function syncLoopSplitter() {
    var el = document.getElementById("loop-splitter");
    if (!el || !slider) return;
    ensureLoopStartInited();
    var max = activeSliderMax();
    var frac = max <= 0 ? 0 : (clampLoopStart(loopStartFrac) / max);
    if (frac < 0) frac = 0;
    if (frac > 1) frac = 1;
  ...
  function fracFromTrackClientX(clientX) {
    ...
    if (t < 0) t = 0;
    if (t > 1) t = 1;
    return t * activeSliderMax();
  }

  function applyScrubPlayhead(dragged) {
    ensureLoopStartInited();
    var d = Number(dragged);
    if (!isFinite(d)) d = playheadFrac;
    var ls = clampLoopStart(loopStartFrac);
    if (d < ls) {
      /* pull splitter left with the dot; never leave playhead left of splitter */
      loopStartFrac = clampLoopStart(d);
      playheadFrac = loopStartFrac;
      persistLoopStart();
      syncLoopSplitter();
    } else {
      /* right never moves splitter */
      playheadFrac = clampFrac(d);
    }
  }
...
/* === BEGIN CLOUD STATIC + PLAY IMAGE OVERLAY (opt11: clouds under beaches) === */
  ...
  var WMS_MAX_SIDE_TABLET = 900;
  var WMS_MAX_SIDE_DESKTOP = 1024;
  var WMS_MAX_SIDE_DESKTOP_HI = 1024;
  /* opt50: soft preview caps above; hi-res progressive enhance */
  var HI_RES_MAX_SIDE = 1920;
  var HI_RES_FADE_MS = 400;
  var PLAY_HIRES_LRU_CAP = 36;
  var GIBS_SRC = "gibs";
  var GIBS_LAYER = "gibs";
  ...
  var loopStartFrac = 0; /* opt49: continuous; Loop rewind / Play window start */
  var loopStartInited = false;
  var loopSplitterDragging = false;
  var savedLoopStartFrac = _sunnySavedLoopStartFrac; /* 0–1 from localStorage, applied when timeline ready */
  var playRafId = 0;
  var playRafLastMs = 0;
  ...
  var scrubDesiredIdx = null; /* opt38: coalesce slider input to one apply/frame */
  var scrubRaf = 0;
  var mapGesturing = false; /* opt38: pan/zoom — pause heavy idle work */
  var idleGoesWarmTimer = null; /* opt38: quiet nearby WMS warm into playImgCache */
  var IDLE_GOES_WARM_MS = 1700;
  var IDLE_GOES_WARM_COUNT = 3;
  var timeTicksSig = "";
  ...
  var playHiResCache = Object.create(null); /* opt50: separate LRU for full-res WMS */
  var playHiResOrder = [];
  ...
  var loadingFrameHint = false; /* download hint active */
  var blendingFrameHint = false;
  ...
  function playLoopOn() {
    if (toggleLoop) return !!toggleLoop.checked;
    return !!playLoop;
  }
  ...
  function releasePlayStaticHold() {
    if (!playHoldStaticUntilFirst) return;
  ...
  function cloudStackBefore() {
    /* Insert cloud rasters under radar (opt16/opt40 single buffer), else Labels, else beaches */
  ...
  function cloudRasterPaint(opacity, layerId) {
    ...
    var fadeDur = (layerId === "iem-vis") ? 320 : HI_RES_FADE_MS; /* opt50: GIBS fade-in over pop */
    return {
      "raster-opacity": opacity == null ? 0 : opacity,
      "raster-resampling": cloudResamplingForLayer(layerId || GIBS_LAYER),
      "raster-fade-duration": fadeDur,
  ...
  function wmsMaxSide() {
    /* opt43 Play WMS: phone ~720, tablet ~900, desktop ~1024; gate by width/DPR */
    ...
    if (phone) {
      ...
      if (dpr >= 2.5) cap = Math.min(cap, 640);
    } else if (tablet) {
      cap = WMS_MAX_SIDE_TABLET;
      if (dpr >= 2.5 && cw < 900) cap = Math.min(cap, 800);
  ...
  function playImageSize() {
    /* opt10: cache WMS size; only recompute on real resize/orientation (debounced).
       opt37: at Speed ≥2 during Play, slightly smaller JPEG for faster WMS (≥2x shrink).
    ...
    /* Clamp min edge ~512 when base allows, else keep ≥64 floor */
    var minEdge = Math.min(w, h);
    var baseMin = Math.min(base.w, base.h);
    if (minEdge < 512 && baseMin >= 512) {
      var up = 512 / Math.max(1, minEdge);
      w = Math.round(w * up);
      h = Math.round(h * up);
    }
  ...
  function computePlayHiResSize() {
    /* opt50: near CSS pixels × DPR, sane max; soft preview stays in computePlayImageSize */
    var el = map && map.getContainer ? map.getContainer() : document.getElementById("map");
    var cw = (el && el.clientWidth) || window.innerWidth || 800;
    var ch = (el && el.clientHeight) || window.innerHeight || 600;
    var dpr = 1;
    try { dpr = Math.min(3, Math.max(1, window.devicePixelRatio || 1)); } catch (eD) { dpr = 1; }
    var w = Math.round(cw * dpr);
    var h = Math.round(ch * dpr);
    var maxSide = Math.max(w, h);
    var cap = HI_RES_MAX_SIDE;
    if (maxSide > cap) {
      var s = cap / maxSide;
      w = Math.max(64, Math.round(w * s));
      h = Math.max(64, Math.round(h * s));
  ...
  function playHiResWorthwhile() {
    var soft = playImageSize();
    var hi = computePlayHiResSize();
    return hi.w > soft.w + 16 || hi.h > soft.h + 16;
  }

  function gibsWmsHiResUrl(iso, lon) {
    var layer = goesLayer(lon == null ? -79 : lon);
    var box = roundPlayBbox(mapBbox3857());
    var sz = computePlayHiResSize();
    return "https://gibs.earthdata.nasa.gov/wms/epsg3857/best/wms.cgi" +
      "?SERVICE=WMS&REQUEST=GetMap&VERSION=1.3.0" +
      "&LAYERS=" + encodeURIComponent(layer) +
      "&STYLES=&FORMAT=image/jpeg&TRANSPARENT=FALSE" +
  ...
  function gibsWmsHiResMeta(iso, lon) {
    var box = roundPlayBbox(mapBbox3857());
    var sz = computePlayHiResSize();
    var url = gibsWmsHiResUrl(iso, lon);
    return { iso: iso, box: box, sz: sz, url: url, key: playFrameKey(iso, box, sz) + "|hi" };
  }

  function playHiResLruTouch(key) {
    var i = playHiResOrder.indexOf(key);
    if (i >= 0) playHiResOrder.splice(i, 1);
    playHiResOrder.push(key);
  }

  function playHiResLruGet(key) {
    var rec = playHiResCache[key];
    if (!rec || !rec.ok) return null;
    playHiResLruTouch(key);
    return rec;
  }

  function playHiResLruSet(key, rec) {
    if (!rec || !rec.ok) return;
    rec.key = key;
    rec.hires = true;
    if (playHiResCache[key]) {
      playHiResCache[key] = rec;
      playHiResLruTouch(key);
      return;
    }
    while (playHiResOrder.length >= PLAY_HIRES_LRU_CAP) {
      var drop = playHiResOrder.shift();
      if (drop && playHiResCache[drop]) delete playHiResCache[drop];
    }
    playHiResCache[key] = rec;
    playHiResOrder.push(key);
  }

  function loadHiResImageSrc(url, cacheKey) {
    var key = cacheKey || url;
    if (tabHidden) return Promise.resolve({ ok: false, url: url, img: null, key: key, cached: false });
    var hit = playHiResLruGet(key);
    if (hit) {
      hit.cached = true;
      return Promise.resolve(hit);
    }
    if (playHiResPending[key]) return playHiResPending[key];
    var p = new Promise(function (resolve) {
      if (tabHidden) {
        resolve({ ok: false, url: url, img: null, key: key, cached: false });
        return;
      }
      var img = new Image();
      var settled = false;
      function finish(ok) {
        if (settled) return;
        settled = true;
        delete playHiResPending[key];
        var rec = { ok: !!ok, url: url, img: img, key: key, cached: false, hires: true };
        if (ok) playHiResLruSet(key, rec);
        resolve(rec);
      }
    ...
    playHiResPending[key] = p;
    return p;
  }

  function cachedHiResRec(idx) {
    if (playUseOpenMeteo || !cloudTimes.length || !playHiResWorthwhile()) return null;
    var iso = cloudTimes[clampIndex(idx)];
    if (!iso) return null;
    try {
      var meta = gibsWmsHiResMeta(iso, origin ? origin.lon : null);
      var rec = playHiResLruGet(meta.key);
  ...
  function hidePlayOverlay() {
    ...
    for (i = 0; i < imgs.length; i++) {
      ...
      imgs[i].classList.remove("is-incoming");
      /* keep src so we do not flash empty if re-entered; do not clear */
    }
  }

  function clearPlayFrameStatusIfOurs() {
    try {
      var el = statusEl || document.getElementById("status");
      if (!el) return;
  ...
  function clearPlayFrameHints() {
    if (blendingHintTimer) {
      clearTimeout(blendingHintTimer);
      blendingHintTimer = 0;
    }
    if (loadingFrameHint || blendingFrameHint) {
      loadingFrameHint = false;
      blendingFrameHint = false;
      clearPlayFrameStatusIfOurs();
    }
  }

  function showDownloadingHint(show) {
    /* opt43: only for slow WMS download/decode — never for flow compute */
    if (show && !loadingFrameHint) {
      loadingFrameHint = true;
      blendingFrameHint = false;
      if (blendingHintTimer) { clearTimeout(blendingHintTimer); blendingHintTimer = 0; }
      setStatus("Downloading cloud frame…");
  ...
  function scheduleBlendingHint(fromCachedDownload) {
    /* Prefer only when download was cached and motion blend runs >200ms */
    if (blendingHintTimer) { clearTimeout(blendingHintTimer); blendingHintTimer = 0; }
    if (!fromCachedDownload) return;
    blendingHintTimer = setTimeout(function () {
      blendingHintTimer = 0;
      if (!playMotionActive || loadingFrameHint) return;
      blendingFrameHint = true;
      setStatus("Blending frames…");
    }, BLEND_HINT_MS);
  }

  function clearBlendingHint() {
    if (blendingHintTimer) {
      clearTimeout(blendingHintTimer);
      blendingHintTimer = 0;
    }
    if (blendingFrameHint) {
      blendingFrameHint = false;
      clearPlayFrameStatusIfOurs();
    }
  }
  ...
  function presentStaticGibs(url) {
    ...
    /* opt50: fade-in over pop — keep prior tiles visible; MapLibre raster-fade softens new tiles */
    var targetOp = gibsEffectiveOpacity();
    try {
      m.setPaintProperty(GIBS_LAYER, "raster-fade-duration", HI_RES_FADE_MS);
      m.setPaintProperty(GIBS_LAYER, "raster-opacity", targetOp);
    } catch (eFd) {}
  ...
  function loadImageSrc(url, preferCors, cacheKey) {
    ...
    var p = new Promise(function (resolve) {
      ...
      function finish(ok) {
        ...
        var rec = { ok: !!ok, url: url, img: img, key: key, cached: false };
  ...
  function settleMotionToFront() {
    ...
    try {
      if (iso && !playUseOpenMeteo) {
        ...
        try {
          var hiMeta = gibsWmsHiResMeta(iso, origin ? origin.lon : null);
          var hiRec = playHiResLruGet(hiMeta.key);
          if (hiRec && hiRec.ok && hiRec.url) rec = hiRec;
        } catch (eHiSettle) {}
  ...
  function crossfadePlayImage(url, cacheKey) {
    return new Promise(function (resolve) {
      ...
      /* Arm download hint only when not already in LRU (prefetch honesty) */
      var keyProbe = cacheKey || url;
      var alreadyCached = !!(keyProbe && playImgCache[keyProbe] && playImgCache[keyProbe].ok);
      var stuckTimer = 0;
      if (!alreadyCached) {
        stuckTimer = setTimeout(function () {
          if (gen === playLoadGen) showDownloadingHint(true);
        }, FRAME_LOAD_STUCK_MS);
      }
      ...
      function reveal() {
        ...
        setPlayLayerOpacity();
        /* opt43: motion if flow cached; else opacity (no ~800ms A-hold) */
        beginPlayTransition(back, front, gen, cacheKey || back.src, function (ok) {
          if (ok) {
            ...
            try { prefetchPlayIndices(cloudIndex); } catch (ePi) {}
          }
          resolve(ok);
        });
      }
      ...
      loadImageSrc(url, false, cacheKey).then(function (rec) {
        ...
        playFrameFromCache = !!(rec.cached || alreadyCached);
        if (playFrameFromCache) {
          /* Cache hit: never show download hint */
          if (stuckTimer) clearTimeout(stuckTimer);
          showDownloadingHint(false);
        }
  ...
  function cancelIdleGoesWarm() {
    if (idleGoesWarmTimer) {
      clearTimeout(idleGoesWarmTimer);
      idleGoesWarmTimer = null;
    }
  }

  function scheduleIdleGoesWarm() {
    cancelIdleGoesWarm();
    if (tabHidden || playMode || mapGesturing || scrubbing || radarOn()) return;
    if (!cloudTimes.length || playUseOpenMeteo) return;
    idleGoesWarmTimer = setTimeout(function () {
      idleGoesWarmTimer = null;
      warmIdleGoesCache();
    }, IDLE_GOES_WARM_MS);
  }

  function warmIdleGoesCache() {
    /* opt38: quiet 2–4 nearby GOES WMS into playImgCache while idle */
    if (tabHidden || playMode || mapGesturing || scrubbing || radarOn()) return;
    if (!cloudTimes.length || playUseOpenMeteo) return;
    try {
      if (map && (map.isMoving() || map.isZooming())) return;
    ...
    var warmed = 0;
    var oi;
    for (oi = 0; oi < offsets.length && warmed < IDLE_GOES_WARM_COUNT; oi++) {
      var idx = clampIndex(from + offsets[oi]);
  ...
  function enterPlayMode() {
    ...
    ensurePlayLayer();
    showPlayOverlay();
    /* opt21: keep last static GIBS visible under play layer until first cover completes */
  ...
  function setCloudFrame(i, opts) {
    ...
    var p;
    var livePlay = playMode && !scrubbing;
    if (livePlay) {
      /* opt38: kick prefetch as soon as reveal/load starts — not only after shown */
      prefetchPlayIndices(cloudIndex);
...
/* === END CLOUD PATCH SNIPPET === */
  ...
  function maybeIdlePrefetchRing() {
    if (mapGesturing || idlePrefetchBusy || tabHidden || !beachManifest || !beachManifest.regions) return;
  ...
  function weatherCollectSunny(list, assumeForeign, wantCount, genToken) {
    ...
    function step() {
      ...
      /* opt38: don't start a new sunny weather batch mid pan/zoom */
      if (mapGesturing) {
        return new Promise(function (resolve) {
  ...
  function applyRadarPastKeep(keepMs) {
    /* Only remap loopStartFrac onto radar indices while Radar overlay is active */
    var keepLoopMs = (loopStartInited && typeof radarOn === "function" && radarOn())
      ? loopStartMs() : NaN;
    ...
    if (radarPast.length && typeof radarOn === "function" && radarOn()) {
      if (isFinite(keepLoopMs)) {
        var lb = 0, ld = Infinity, j, jms, jd;
        for (j = 0; j < radarPast.length; j++) {
          if (!radarPast[j] || radarPast[j].time == null) continue;
          jms = Number(radarPast[j].time) * 1000;
          jd = Math.abs(jms - keepLoopMs);
          if (jd < ld) { ld = jd; lb = j; }
        }
        loopStartFrac = lb;
        loopStartInited = true;
  ...
  function radarParkedOffNow() {
    return !!(radarPast.length && radarIndex < radarSliderMax());
  }
  ...
  function startRadarRefresh() {
    ...
    /* opt28: parked on a historical radar frame — do not jump to latest */
    if (radarParkedOffNow()) return;
  ...
  function setRadarFadeDuration(ms) {
    ...
    try { map.setPaintProperty(RADAR_LAYER, "raster-fade-duration", n); } catch (eFd) {}
  }

  function ensureRadarLayer(url, opts) {
    opts = opts || {};
    if (!map || !url) return;
    var before = radarStackBefore();
    var fade = opts.fadeMs;
    if (fade == null) {
      fade = (isPlaying() && radarOn()) ? playFadeMs() : 0;
    }
    ...
    /* Drop legacy dual buffer if an old session left it (opt39) */
    try {
      if (map.getLayer("radar-b")) map.removeLayer("radar-b");
    } catch (eLb) {}
    try {
      if (map.getSource("radar-b")) map.removeSource("radar-b");
    } catch (eSb) {}
    restackCloudRasters();
    restackRadar();
  }
    ...
    return fetch(RADAR_MANIFEST_URL, { cache: "no-store" }).then(function (res) {
      ...
      if (opts.jumpLatest || radarIndex > radarSliderMax()) {
        ...
        for (k = 0; k < radarPast.length; k++) {
          if (radarPast[k] && radarPast[k].time === keepTime) {
            match = k;
            break;
          }
      ...
      radarPath = frame.path;
      ensureRadarLayer(radarTileUrl(data.host, frame.path), { fadeMs: 0 });
      syncRadarTimelineUi();
      return true;
    }).catch(function () {
  ...
  function setHoursBack(hours, opts) {
    ...
    if (isFinite(keepLoopMs) && cloudTimes.length) {
  ...
  function startRadarPlay(opts) {
    ...
    /* opt49: from Now/end jump to loop-start splitter (even Loop off — play window then stop) */
  ...
  function stopPlay() {
    /* Pause button: fully stop. Do not resume after an in-flight scrub. */
  ...
  function startPlay(opts) {
    ...
    /* opt49: jump to loop-start splitter only when starting from end/Now; else resume. */
  ...
  map.on("moveend", function () {
    ...
    moveTimer = setTimeout(function () {
      /* opt38: resume heavy work only after gesture quiet (MOVE_DEBOUNCE) */
  ...
  map.on("load", function () {
    ...
    if (!radarOn()) {
      /* opt40: do NOT paint WMTS for optimistic tip — wait for probe (avoids first-paint 404 storm) */
      try {
        if (timeLabel && cloudTimes.length) {
          timeLabel.textContent = formatFrameLabel(cloudTimes[cloudTimes.length - 1]) + " · …";
        }
      } catch (eLbl) {}
      resolveGoesLatestIso(origin ? origin.lon : -79).then(function (iso) {
        ...
        /* probe failed — fall back to conservative wall tip once */
        if (!playMode && !tabHidden && !radarOn()) {
          var fb = goesLatestIsoSync();
          applyGoesLatestToTimeline(fb);
          presentStaticGibs(gibsTiles(fb, origin ? origin.lon : -79));
          setCloudFrame(cloudIndex);
        }
      });
    } else {
  ...
  function applyScrubFrame() {
    ...
    applyScrubPlayhead(fracFromSliderUi(raw));
    if (radarOn()) {
  ...
  function flushScrubFrame() {
    /* opt38: cancel pending rAF and apply final index immediately */
    if (scrubRaf) {
      try { cancelAnimationFrame(scrubRaf); } catch (eCaf) {}
      scrubRaf = 0;
    }
    if (scrubDesiredIdx == null) scrubDesiredIdx = Number(slider.value);
    applyScrubFrame();
  }
  ...
  function scheduleScrubFrame() {
    ...
    scrubRaf = requestAnimationFrame(function () {
      scrubRaf = 0;
      if (scrubDesiredIdx == null) return;
      applyScrubFrame();
    });
  }
  ...
  function endScrub() {
    if (!scrubbing) return;
    scrubbing = false;
    var was = scrubWasPlaying;
    scrubWasPlaying = false;
    flushScrubFrame();
    cancelIdleGoesWarm();
    if (was) {
      startPlay({ resume: true });
    } else if (radarOn() && !tabHidden && radarIndex >= radarSliderMax()) {
      startRadarRefresh();
    } else {
      scheduleIdleGoesWarm();
    }
  ...
  /* opt49: loop-start splitter drag */
  (function bindLoopSplitter() {
    var el = document.getElementById("loop-splitter");
    if (!el) return;
    var pid = null;
    function onDown(ev) {
      ...
      ev.preventDefault();
      ev.stopPropagation();
      loopSplitterDragging = true;
      el.classList.add("is-dragging");
      pid = ev.pointerId != null ? ev.pointerId : null;
      try { if (pid != null) el.setPointerCapture(pid); } catch (eC) {}
      ensureLoopStartInited();
      setLoopStartFrac(fracFromTrackClientX(ev.clientX));
      if (radarOn()) {
        setRadarFrame(clampRadarIndex(Math.round(playheadFrac)));
      } else if (cloudTimes.length) {
        setCloudFrame(playheadFrac, { forceHires: true, frac: playheadFrac });
      }
    }
    function onMove(ev) {
      if (!loopSplitterDragging) return;
    ...
    el.addEventListener("keydown", function (ev) {
      ensureLoopStartInited();
      var max = activeSliderMax();
      var step = Math.max(0.25, max / 100);
      if (ev.key === "ArrowLeft" || ev.key === "ArrowDown") {
        ev.preventDefault();
        setLoopStartFrac(loopStartFrac - step);
  ...
  slider.addEventListener("mousedown", beginScrub);
  ...
  slider.addEventListener("input", function () {
    ...
    applyScrubPlayhead(fracFromSliderUi(scrubDesiredIdx));
    /* reflect clamp/pull back onto native range */
    slider.value = String(sliderUiFromFrac(playheadFrac));
    syncPlayheadThumb();
    scheduleScrubFrame();
  });
  ...
  document.addEventListener("visibilitychange", function () {
    if (document.hidden) {
      ...
      if (scrubbing) {
        ...
        scrubWasPlaying = false;
        if (scrubRaf) {
          try { cancelAnimationFrame(scrubRaf); } catch (eScrR) {}
          scrubRaf = 0;
        }
        scrubDesiredIdx = null;
      }
